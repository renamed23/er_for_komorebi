...
try:
    ...
    HAS_WORDFREQ = False
    print("警告: 未检测到 wordfreq 或相关依赖，将跳过生僻字过滤。")


def is_game_processable_char(ch: str) -> bool:
    """
    判断单个字符是否能被该游戏文本逻辑按“可绘制双字节字形”处理。

    规则来自反编译（402550/402D70/402940）：
    - 必须能编码为 CP932 双字节；
    - 首字节必须在 0x81..0x9F 或 0xE0..0xE9。

    说明：
    - 像“黯”(EA7E) 会返回 False（EA 不在允许范围）。
    - 返回 True 仅表示“解析路径可达字形绘制分支”，不保证该槽位里一定有你想要的字形。
    """
    ...
    if len(ch) != 1:
        raise ValueError(f"必须是单个字符，收到: {ch}")
    ...
    lead = b[0]
    return (0x81 <= lead <= 0x9F) or (0xE0 <= lead <= 0xE9)
...
class EncodingType(Enum):
    ...
    def contains_char(self, ch: str) -> bool:
        """检查字符是否能被该编码表示"""
        return is_game_processable_char(ch)
...
# 替身池生成
# -----------------------------
def generate_pool(paths: list[Path], output: Path, encoding: EncodingType, filter_rare: bool = False):
    pool = set()
    ...
    for start, end in encoding.suggested_ranges():
        for code in range(start, end + 1):
            ch = chr(code)
            if filter_rare and HAS_WORDFREQ and encoding in (EncodingType.CP932, EncodingType.SHIFT_JIS):
                # 仅保留常用字符
                if encoding.contains_char(ch) and word_frequency(ch, 'ja') > 1e-6:
                    pool.add(ch)
            else:
